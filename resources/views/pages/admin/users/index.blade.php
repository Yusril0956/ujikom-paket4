<x-layouts.app>
    <div class="max-w-7xl mx-auto space-y-6 px-2">
        ...
        @elseif (session('error'))
            <div class="bg-red-50 border border-red-300 rounded-md p-4 flex items-start gap-3">
                <x-lucide-x-circle class="w-5 h-5 text-red-600 mt-0.5 flex-shrink-0" />
                <p class="font-serif text-sm text-red-800">{{ session('error') }}</p>
            </div>
        @endif
                    ...
                    <tbody class="divide-y divide-ink">
                        @forelse($users as $user)
                            <tr class="hover:bg-ink/5 transition-colors">
                                <td class="px-6 py-4 font-mono text-coffee font-semibold">{{ $user->formatted_id }}
                                </td>
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex items-center justify-end gap-2">
                                        ...
                                        @if (auth()->user()->role === 'petugas' && $user->role_label == 'Anggota')
